...
.btn-primary:hover:not(:disabled) {
  background: #466eff;
  box-shadow: 0 4px 14px rgba(70, 110, 255, 0.35);
}
...
- **Backend Rust**:
  - File utama: `rust-backend/src/main.rs` — mendefinisikan route `POST /process` (handler `process_image`) dan route `GET /` (root). 
...
- **Detektor Python**:
  - File utama: `python-detector/app.py` — Flask app dengan endpoint `POST /detect`.
